...
export default function AdminGalleryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setCollections(rows);
    setPhotoCounts(
      Object.fromEntries(rows.map((row) => [row.id, row.gallery_photos?.[0]?.count ?? 0])),
    );
    setIsLoading(false);
  }, [supabase]);
  ...
  async function addCollection() {
    ...
    if (!slug) return setAddError("That name doesn't produce a usable URL — add a letter or number.");
    if (collections.some((collection) => collection.slug === slug))
      return setAddError(`A collection with the slug "${slug}" already exists.`);

    setAddError("");
    const nextOrder = Math.max(0, ...collections.map((c) => c.sort_order)) + 10;
    const { error: insertError } = await supabase.from("gallery_collections").insert({
      slug,
      title,
      sort_order: nextOrder,
    });
    if (insertError) return toast.error("Could not create the collection.", { description: insertError.message });
    setNewTitle("");
  ...
  async function saveCollection(
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", collection.id);
    if (updateError)
      return toast.error("Could not save that collection.", { description: updateError.message });
  ...
  async function togglePublished(collection: GalleryCollection) {
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", collection.id);
    if (updateError)
      return toast.error("Could not update that collection.", { description: updateError.message });
    await afterWrite(
  ...
  async function toggleShowOnHome(collection: GalleryCollection) {
    ...
    const homeSortOrder = turningOn
      ? Math.max(0, ...collections.filter((c) => c.show_on_home).map((c) => c.home_sort_order)) + 10
  ...
  async function moveCollection(collection: GalleryCollection, direction: -1 | 1) {
    if (!supabase) return;
  ...
  async function moveHomeCollection(collection: GalleryCollection, direction: -1 | 1) {
    if (!supabase) return;
  ...
  async function setCover(collection: GalleryCollection, photo: GalleryPhoto) {
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", collection.id);
    if (updateError)
      return toast.error("Could not set the cover photo.", { description: updateError.message });
  ...
  async function deleteCollection(collection: GalleryCollection) {
    ...
    const { error: deleteError } = await supabase
      ...
      .eq("id", collection.id);
    if (deleteError)
      return toast.error("Could not delete that collection.", { description: deleteError.message });
  ...
  async function savePhoto(
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", photo.id);
    if (updateError)
      return toast.error("Could not save that photo.", { description: updateError.message });
    await afterWrite("Photo updated.");
  }

  async function movePhoto(collection: GalleryCollection, photo: GalleryPhoto, direction: -1 | 1) {
    if (!supabase) return;
    ...
    const [first, second] = await Promise.all([
      supabase.from("gallery_photos").update({ sort_order: swapWith.sort_order }).eq("id", photo.id),
      supabase.from("gallery_photos").update({ sort_order: photo.sort_order }).eq("id", swapWith.id),
    ]);
    const failure = first.error ?? second.error;
    if (failure)
      return toast.error("Could not reorder.", {
  ...
  async function deletePhoto(collection: GalleryCollection, photo: GalleryPhoto) {
    if (!supabase) return;
    ...
    if (deleteError)
      return toast.error("Could not delete that photo.", { description: deleteError.message });

    if (collection.cover_image_url === photo.image_url) {
      const remaining = (photosByCollection[collection.id] ?? []).filter((p) => p.id !== photo.id);
      const nextCover = remaining.sort((a, b) => a.sort_order - b.sort_order)[0]?.image_url ?? null;
      await supabase
        .from("gallery_collections")
  ...
  async function uploadPhotos(collection: GalleryCollection, files: FileList) {
    if (!supabase) return;
    const { data: userData } = await supabase.auth.getUser();
    if (!userData.user) return toast.error("Please sign in again before uploading.");
    ...
    const uploadToast = toast.loading(
      list.length === 1 ? "Uploading photo..." : `Uploading ${list.length} photos...`,
    );

    let nextOrder = Math.max(0, ...(photosByCollection[collection.id] ?? []).map((p) => p.sort_order));
    ...
    await revalidateGallery();
    if (failures.length) {
      uploadToast.error(summary, { description: `Failed: ${failures.join("; ")}` });
    } else {
...
function CollectionCard({
  ...
  onSavePhoto: (photo: GalleryPhoto, changes: { title: string; caption: string }) => void;
  onMovePhoto: (collection: GalleryCollection, photo: GalleryPhoto, direction: -1 | 1) => void;
    ...
    <div className="rounded-2xl border border-[#06131D]/10 bg-white shadow-sm">
      <div className="flex flex-wrap items-center gap-3 p-4">
        ...
        <button
          ...
          title={collection.is_published ? "Published — visible on /gallery" : "Hidden from the site"}
        ...
        <button
          ...
          title={collection.show_on_home ? "Shown on the home page" : "Not shown on the home page"}
      ...
      {isExpanded && (
        <div className="border-t border-stone-200 bg-[#FAF8F5] p-4">
          <div className="flex items-center justify-between gap-3">
            <p className="font-body text-sm font-semibold text-[#06131D]">Photos</p>
          ...
          <div className="mt-4 space-y-2">
            {photos.map((photo, index) => (
              <PhotoRow
                ...
                onSave={onSavePhoto}
                onMove={(direction) => onMovePhoto(collection, photo, direction)}
...
function PhotoRow({
  ...
  isFirst: boolean;
  isLast: boolean;
  onSave: (photo: GalleryPhoto, changes: { title: string; caption: string }) => void;
  ...
  const isDirty = title.trim() !== photo.title || caption.trim() !== photo.caption;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-stone-200 bg-white p-2.5">
      ...
      <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded-lg bg-[#F3EFEA]">
        <Image src={photo.image_url} alt={photo.title || "Gallery photo"} fill sizes="56px" className="object-cover" />
      </div>

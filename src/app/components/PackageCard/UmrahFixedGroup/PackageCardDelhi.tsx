...
export default function PackageCardDelhi({
  handleBookNow,
}: {
  handleBookNow: () => void;
}) {
  return (
    ...
    >
      ...
      <div className="relative h-60 w-full overflow-hidden bg-stone-900">
        <Image
          src="/packages/umrah/umrah4.jpg"
          alt="15 Days Regular Umrah from Delhi"
          fill
        ...
        <div className="absolute top-3 left-3 right-3 flex justify-between items-center">
          <span className="px-3 py-1 rounded-full text-xs font-bold text-white bg-indigo-700/90 backdrop-blur-md shadow-sm border border-indigo-500/30 flex items-center gap-1">
            <FaPlane className="w-3 h-3" /> Direct Delhi Flight
          </span>
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#06131D]/85 backdrop-blur-md text-[#F3E5AB] text-xs font-medium border border-[#D4AF37]/30">
          ...
          <span>Clock Tower / Steps from Haram</span>
        </div>
      ...
      <div className="p-6 flex flex-col flex-1 justify-between">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 py-3 border-y border-stone-100 text-xs text-stone-600 mb-4">
            ...
            <span className="flex items-center gap-1.5 font-medium">
              <FaShieldAlt className="text-[#D4AF37] w-3 h-3" /> Visa + Guide
            </span>
          </div>
        </div>
